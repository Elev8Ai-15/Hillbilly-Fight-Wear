...
export const PRICING = {
  ...
  // Promotions
  PROMO_TSHIRT_TANK_BUNDLE: true,            // 2 T-Shirts/Tanks for a flat bundle price (scales in pairs)
  PROMO_TSHIRT_TANK_BUNDLE_PRICE: 50,         // Flat price per pair of T-Shirts/Tanks
...
export function calculateCartPricing(cartItems: CartItem[]): PricingBreakdown {
  ...
  // PROMO 1: 2 T-Shirts/Tanks for $50 (flat price per pair)
  // Discount = (regular price of items in pairs) − (pairs × bundle price)
  // Pairs are formed from the most-expensive items first to maximize customer savings.
  if (PRICING.PROMO_TSHIRT_TANK_BUNDLE) {
    ...
    const pairCount = Math.floor(totalTshirtTankQty / 2) // every 2 form a bundle pair
    ...
    if (pairCount > 0) {
      ...
      if (pairDiscount > 0) {
        ...
        discountDetails.push({
          ...
          description: `2 for $${PRICING.PROMO_TSHIRT_TANK_BUNDLE_PRICE} (T-Shirts & Tanks) - ${pairCount} pair${pairCount > 1 ? 's' : ''}`,
